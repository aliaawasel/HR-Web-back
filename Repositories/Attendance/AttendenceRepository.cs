...
namespace HR_System.Repositories.Attendance
{
    public class AttendenceRepository : IAttendenceRepository
    {
        ...
        {
            var Attendances = hREntity.Attendances.Include(a => a.Employee).ThenInclude(e => e.Department).Where(a => a.IsDeleted != true);
        ...
        public List<AttendenceDto> GetByName(string name)
        {
            ...
            return Attendances.Where(e => e.EmployeeName.Contains(name)).ToList();
        }
    ...
    //    public List<Models.Attendance> xxx(int month, int year, int id)
    //    {
    //        int absenceDays = 0;

    //        for (var day = 1; day <= NumberOfDays(month, year); day++)
    //        {
    //            var Attendances = hREntity.Attendances.Include(a => a.Employee).Where(e => e.EMPId == id && e.Date.Month == month && e.Date.Year == year && e.Date.Day == day && e.IsDeleted != true).FirstOrDefault();
    //            if (Attendances == null)
    //            {
    ...
    //                    if (generalSettingsRepository.IFWeeklyVaction(day, month, year) == 1)
    //                    {
    //                        absenceDays++;
    //                    }
    //                }
